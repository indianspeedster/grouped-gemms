...
print(f"{'Shape':>25s}  {'native':>8s}  {'fast':>8s}  {'v2':>8s}  {'gluon':>8s}  {'best':>8s}")
print("-" * 75)

geo_nat, geo_fast, geo_v2, geo_glu = [], [], [], []
...
for e, m, n, k, label in shapes:
    ...
    best = max(nat_tf, fast_tf, v2_tf, glu_tf)
    print(f"({e},{m//1000:>3d}K,{n},{k}) {label:>7s}  {nat_tf:>6.0f}  {fast_tf:>6.0f}  {v2_tf:>6.0f}  {glu_tf:>6.0f}  {best:>6.0f}")

n = len(shapes)
print(f"\nGeomean TFLOPS:  native={math.exp(sum(geo_nat)/n):.0f}  fast={math.exp(sum(geo_fast)/n):.0f}  v2={math.exp(sum(geo_v2)/n):.0f}  gluon={math.exp(sum(geo_glu)/n):.0f}")
print(f"vs peak (5230 TFLOPS):  native={math.exp(sum(geo_nat)/n)/5230*100:.1f}%  fast={math.exp(sum(geo_fast)/n)/5230*100:.1f}%  v2={math.exp(sum(geo_v2)/n)/5230*100:.1f}%  gluon={math.exp(sum(geo_glu)/n)/5230*100:.1f}%")
