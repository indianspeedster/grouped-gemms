"""MXFP8 forward sweep worker. One shape, full config grid, writes JSON.

Mirrors tune_worker_fp4.py but for the MXFP8 (e4m3) kernel. fp8 operands are
1 byte/element (not packed), so BLOCK_K caps at 256 — a 256-K fp8 tile is the
same LDS footprint as a 512-K fp4 tile.

I/O contract: prints OK to stdout, writes incremental JSON to <out_path>.
"""
import argparse
import json
import random
...
def build_search_space():
    """384 configs, kpack=1. Same grid as the fp4 worker but BLOCK_K in
    {128, 256} (fp8 is unpacked)."""
    cfgs = []
